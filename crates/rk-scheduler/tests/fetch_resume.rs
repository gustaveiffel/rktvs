...
async fn fetch_file_with_resume() {
    ...
    assert_eq!(
        fetch_result.skipped_chunks, 2,
        "first 2 chunks should be skipped"
    );
    ...
    let chunks = sat_catalog
        .get_file_chunks("lib-a", "tape-1", "/bigfile.bin")
        .unwrap();
